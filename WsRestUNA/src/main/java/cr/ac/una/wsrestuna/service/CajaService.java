...
//    Obetener una caja
    public Respuesta getCaja(Long id) {
        try {
            ...
            return new Respuesta(true, CodigoRespuesta.CORRECTO, "", "", "Caja", new CajaDto((Caja) qryCaja.getSingleResult()));
...
//    Obtener un listado de todas las cajas
    public Respuesta getCajas() {
        try {
            ...
            List<CajaDto> CajaDto = new ArrayList<>();
            cajas.forEach(caja
                    -> {
                CajaDto.add(new CajaDto(caja));
            });
